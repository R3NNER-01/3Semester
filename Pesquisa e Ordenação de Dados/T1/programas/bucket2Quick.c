...
/* Função auxiliar para escolher um pivô aleatório */
int escolhePivoAleatorio(int low, int high) {
    return low + rand() % (high - low + 1);
}

/* Função auxiliar do Quick Sort para trocar elementos */
...
int particao(int *array, int low, int high) {
    /* Escolhe um índice aleatório e troca com o último elemento */
    int random = escolhePivoAleatorio(low, high);
...
/* Quick Sort para ordenar os buckets */
...
void bucketSort(int *array, int n) {
    ...
    for (i = 0; i < numBuckets; i++) {
        if (buckets[i].tam > 0) {
            quickSort(buckets[i].array, 0, buckets[i].tam - 1);
        }
